  ...
  def pageWithSummarySubscriptionComponent(view: SummarySubscriptionsView, messageKeyPrefix: String): Unit = {
    ...
    "subscription summary component" when {
      ...
      "has no subscriptions added" must {
        ...
        val applyView = view.apply(subscriptions, npsData, navigator.nextPage(SummarySubscriptionsPage, NormalMode, someUserAnswers).url, NormalMode)(fakeRequest, messages)
        val doc: Document = asDocument(applyView)
        ...
        "render change link when nps data present" in {
          ...
          assert(doc.getElementById(taxYear.toString).getElementsByTag("a").eq(0).text() contains
            messages("summarySubscriptions.link.hiddenTextChange", messages(s"taxYearSelection.$taxYear", getTaxYear(taxYear).toString, (getTaxYear(taxYear) + 1).toString)))
        ...
        "render add link when no nps data" in {
          ...
          assert(doc.getElementById(taxYear.toString).getElementsByTag("a").eq(0).text() contains
            messages("summarySubscriptions.link.hiddenTextAdd", messages(s"taxYearSelection.$taxYear", getTaxYear(taxYear).toString, (getTaxYear(taxYear) + 1).toString)))
