...
class Navigator @Inject() () {
  ...
  private val routeMap: Page => UserAnswers => Call = {
    ...
    case SubscriptionAmountPage(year, index) =>
      _ => routes.EmployerContributionController.onPageLoad(NormalMode, year, index)
  ...
  private def changeExpensesEmployerPaid(userAnswers: UserAnswers, year: String, index: Int): Call =
    ...
    (
      ...
      case (Some(contrib), Some(subAmount)) if contrib >= subAmount =>
        routes.CannotClaimEmployerContributionController.onPageLoad(CheckMode, year, index)
  ...
  private def changeAmountsAlreadyInCode(userAnswers: UserAnswers): Call =
    userAnswers.get(AmountsAlreadyInCodePage) match {
