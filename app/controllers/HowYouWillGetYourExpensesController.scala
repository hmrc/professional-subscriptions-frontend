...
class HowYouWillGetYourExpensesController @Inject() (
  ...
  def onPageLoad: Action[AnyContent] = identify.andThen(getData).andThen(requireData) { implicit request =>
    ...
    val getCurrentYearAmount: Option[Int] = request.userAnswers
      .get(SummarySubscriptionsPage)
    ...
    (getTaxYears, getCurrentYearAmount) match {
      case (Some(seqTaxYearSelection), Some(subscriptionAmount))
          if seqTaxYearSelection.contains(CurrentYear) && seqTaxYearSelection.length > 1 =>
        Ok(
          currentAndPreviousYearView(
            redirectUrl,
            containsCurrentYearMinus1(seqTaxYearSelection),
